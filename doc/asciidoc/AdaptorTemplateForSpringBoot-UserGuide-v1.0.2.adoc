...
ATB는 다음과 같은 연계패턴으로 구성되어 있습니다.

[cols="1,2,3"]
|===
| 순번 | 구성요소 | 설명
| 1 | 일반 DB TO DB | 송신 DB에서 수신 DB로 데이터를 전송하는 연계 +
[small]_#-마스터/디테일 테이블 연계 기능 추가 v1.0.2부터 지원#_
| 2 | 대용량 DB TO DB | 송신 DB에서 수신 DB로 처리과정을 대용량을 처리할 수 있도록 파일로 변환하여 전송하는 연계 +
[small]_#-마스터/디테일 테이블 연계 기능 추가 v1.0.2부터 지원#_
| 3 | FILE TO FILE | 송신 서버의 저장소 디렉토리에서 수신 서버의 저장소 디렉토리로 파일을 전송하는 연계5
| 4 | FILE TO DB | 송신 서버의 저장소 디렉토리에서 수신 서버의 저장소 디렉토리로 파일을 전송하고 수신 서버의 DB로 데이터를 전송하는 연계
| 5 | FILE TO DB 배치 | 송수신 서버를 분리하지 않고 디렉토리에서 파일을 읽어서 디비로 전송하는 배치 방식의 연계
...
* 이중화 기능 구현(Active/Standby)
* _#인터페이스 관리 페이지(인터페이스 등록/설정,담당자 관리 ,Console) 기능 v1.0.2부터 지원#_
* _#유지보수를 위한 Console 관리 기능 v1.0.2부터 지원_#
...
어댑터는 Publisher(게시자)와 Subscriber(구독자)로 구분되어 연계를 수행합니다.
...
. admin / 1234qwer!@#$ 으로 로그인 합니다.

==== 5.1.1 Adaptor의 구동
NOTE: ESB_DEMO의 Adaptor 설정은 LINUX설정으로 되어있습니다 윈도우환경에서 Adaptor를 구동하고싶다면 환경변수를 다음과 같이 수정하여주세요
...
| detail-data     | ex) true                  | 마스터/디테일 테이블 사용여부 +
_#v1.0.2부터 지원#_
| detail-names    | ex) CHILD,CHILD2          | 디테일 테이블 Alias(List) +
_#v1.0.2부터 지원#_
|===
...
SQL_IF_DTD_001.xml:: Mybatis 설정 파일 (단일 테이블 구조)
...
SQL_IF_DTD_003.xml:: Mybatis 설정 파일 (마스터/디테일 구조) _#v1.0.2부터 지원#_

[source,xml]
----
include::../../TEMPLATE/config/ATB_DTD_S_01/SQL_IF_DTD_003.xml[SQL_IF_DTD_003.xml]
----
...
| description        | ex) "(DB SENDER) + JMS + (DB RECEIVER)" | 연계 설명
| prefix             | ex) DTD             | 식별자
| auto-config        | ex) true            | 인터페이스 관리 설정에서 가져옵니다. (default : false) +
_#v1.0.2부터 지원#_
|===
...
| send-system-code| ex) A01                   | 송신 시스템 코드(이력용)
| receive-system-code | ex) B01               | 수신 시스템 코드(이력용)
| procedure-call  | ex) true                  | 프로시저 호출 여부 (default : false)
|===
...
[source,xml]
----
include::../../TEMPLATE/config/ATB_DTD_R_01/SQL_IF_DTD_003.xml[SQL_IF_DTD_003.xml]
----
...
NOTE: 재처리 기능은 송신 어댑터에서 구현되어 작동되며 재처리 SQL을 실행합니다.
...
NOTE: ★ 송신 어댑터는 스케줄 이벤트에따라 +
1. 출발지(Source) DB 테이블을 연계 상태 컬럼을 N인 데이터를 상태코드를 'P'(처리중)과 트랜젝션ID을 업데이트 2. 트랜젝션ID로 조회한 데이터를 +
3. 임시(Temp) 폴더에 파일을 생성하고 기록합니다. +
4. 생성된 파일을 ESB Hub의 파일서버로 전송(PUT) 하고 +
5. 전송 실패하였다면 에러(Error) 폴더에 파일을 넣고 종료를 합니다.
정상적으로 송신하였다면. +
6. 완료(Complete) 폴더 에 넣고 ESB MQ(MessageQueue) DATA 큐로 파일의 메타정보를 전송합니다. +
★ 수신 어댑터는 ESB MQ(MessageQueue)인 DATA 큐로 부터 +
7. 파일의 메타정보를 수신하고 ESB Hub의 파일서버로 부터 임시(Temp) 폴더에 파일 수신(GET) 을 진행합니다. +
8. 수신 받은 파일을 대상(Target) DB 테이블에 INSERT를 한후 +
9. 옵션에 따라 프로시저를 호출 합니다. +
10. 처리 결과에 따라 파일을 성공(Success) 또는 에러(Error) 폴더에 이동시키고 처리 결과를 ESB MQ(MessageQueue)인 RESULT 큐로 전송합니다. +
★ 송신 어댑터는 +
11. ESB MQ(MessageQueue)로부터 수신하고 처리결과를 출발지(Source) DB 테이블에 UPDATE합니다. +
12. 처리결과를 완료(Complete) 폴더에 있던 파일을 성공(Success) 또는 에러(Error) 폴더에 이동시킵니다.

IMPORTANT: _#기존의 대용량 DB to DB는 파일을 처리할때 송신테이블에 대한 결과처리를 하지 못하였으나 연계 메커니즘(1번,2번)을 변경하여 송신테이블에 결과를 업데이트할수 있도록 변경하였습니다.v1.0.2부터 지원#_
...
SQL_IF_DTD_002.xml:: Mybatis 설정 파일 (단일 테이블 구조)

[source,xml]
----
include::../../TEMPLATE/config/ATB_DFTFD_S_01/SQL_IF_DTD_004.xml[SQL_IF_DTD_004.xml]
----

SQL_IF_DTD_004.xml:: Mybatis 설정 파일 (마스터/디테일 구조) _#v1.0.2부터 지원#_
...
TIP: namespace는 인터페이스 ID를 넣고 TX_UPDATE , SELECT , RESULT_UPDATE,SELECT_RESEND 를 추가합니다.
기존의 (SELECT , UPDATE, RESULT_UPDATE , SELECT_RESEND) 였던것을 변경함 _#v1.0.2부터 지원#_
...
★ 수신 어댑터는 ESB MQ(Message Queue)인 DATA 큐로 부터 +
...
== 14 인터페이스 등록 및 Adaptor 연동

Application.yml에 인터페이스 설정을 인터페이스 등록/조회를 통하여 등록을 할수 있게 되었습니다. _#v1.0.2부터 지원#_

=== 14.1 auto-config 활성화
context 설정을 제거하고 auto-config를 true로 설정하면 Adaptor 구동시에 인터페이스 설정을 가져옵니다. +
아래는 설정 예제입니다.
...
atb:
  if-config:
    id: DTD_SENDER_01
    description: "(DB SENDER) + JMS + (DB RECEIVER)"
    prefix: DTD
    # 인터페이스 관리 설정에서 가져옵니다.
    auto-config: true
spring:
...
인터페이스 등록은 인터페이스 등록/조회 화면으로 이동후 아래와 같이 등록을 합니다.

==== 14.2.1 추가 버튼 선택
image::img/GUI_INTERFACE_LIST.png[]

==== 14.2.2 인터페이스 정보 입력
image::img/GUI_INTERFACE_ADD1.png[]

==== 14.2.3 송신자 어댑터 추가 -> 송신자 환경설정
image::img/GUI_INTERFACE_ADD_CONFIG1.png[]

==== 14.2.4 어뎁터 지정 -> 환경설정(템플릿) 수정

image::img/GUI_INTERFACE_ADD_CONFIG2.png[]

==== 14.2.5 수신자 어댑터 추가 -> 수신자 환경설정

image::img/GUI_INTERFACE_ADD_CONFIG4.png[]
