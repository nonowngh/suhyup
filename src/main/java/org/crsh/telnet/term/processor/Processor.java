...
public final class Processor implements Runnable, ScreenContext {
    ...
    CrashLogin crashLogin = new CrashLogin();
...
//        try {
        //로그인 인증 쉘
        ShellProcess process = shell.createProcess("auth");
        current = new ProcessContext(this, process);

        current.run();

        status = Status.PROCESSING;
        while (Status.PROCESSING.equals(status)) {
            try {
                Thread.sleep(200);
...
//        }
    ...
    void writePromptFlush() {
        ...
        try {
            //로그인 구현
            if (!isLogin) {
                if (crashLogin.isLogin(Thread.currentThread().getId())) {
                    isLogin = true;
                } else {
                    log.debug("Please login first.\r\n");
                    term.close();
                    isLogin = false;
                }
            }
