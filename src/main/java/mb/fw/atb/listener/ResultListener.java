...
public class ResultListener implements MessageListener {
    ...
    public void onMessage(Message message) {
        log.info("RESULT LISTENER START");
        ...
        try{
            ...
            log.info("JMS LISTENER STARTED ");
            log.debug("{} ", context);
            log.info("Receiver's Property : {}",  propMap);
            log.info("Receiver's Data : {}",  json);

            try {
                String resultCode = propMap.get(THeader.RECEIVER_RESULT_CD.key());
                String resultMessage = propMap.get(THeader.RECEIVER_RESULT_MSG.key());
                ...
                strategy.onMessageResult(context, txid, msgRcvDt, resultCode, resultMessage, json, propMap);
            } catch (Exception e) {
                log.error("onMessageResult Error", e);
            }
            log.info("RESULT LISTENER END");
