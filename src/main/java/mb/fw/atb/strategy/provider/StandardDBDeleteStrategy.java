...
import mb.fw.atb.schema.provider.StandardDBDeleteStrategySchema;
...
@Component(value = "StandardDBDeleteStrategy")
@Slf4j
public class StandardDBDeleteStrategy extends ATBStrategy {
    ...
    public Specifications specifications() {
        return StandardDBDeleteStrategySchema.specifications();
    }
    ...
    public Object onMessageData(IFContext context, String txid, String eventDt, Object data, Map<String, String> propMap) throws Exception {
        ...
        try {
            dbTransactionService.updateList(context.getInterfaceId() + ".DELETE", context, dataList, propMap);
        } catch (Exception e) {
            ErrorCode errorCode = ErrorCode.DELETE_FAILURE;
            ATBException atbException = new ATBException("DB DELETE FAILURE", errorCode, e.getCause());
            ...
            propMap.put(THeader.RECEIVER_ADAPTOR_NAME.key(), adaptorConfigBean.getAdaptorName());
            propMap.put(THeader.RECEIVER_RESULT_CD.key(), TResult.FAIL.value());
            propMap.put(THeader.RECEIVER_RESULT_MSG.key(), "DB DELETE FAILURE");
            retMap.put(THeader.IF_RESULT.key(), propMap);
            return retMap;
        }
