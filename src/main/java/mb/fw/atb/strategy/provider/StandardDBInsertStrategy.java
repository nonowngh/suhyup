...
import mb.fw.atb.schema.provider.StandardDBInsertStrategySchema;
...
@Component(value = "StandardDBInsertStrategy")
@Slf4j
public class StandardDBInsertStrategy extends ATBStrategy {
    ...
    public Specifications specifications() {
        return StandardDBInsertStrategySchema.specifications();
    }
    ...
    public Object onMessageData(IFContext context, String txid, String eventDt, Object data, Map<String, String> propMap) throws Exception {
        ...
        try {
            dbTransactionService.insertList(context.getInterfaceId() + ".INSERT", context, dataList, propMap);
        } catch (Exception e) {
            ErrorCode errorCode = ErrorCode.INSERT_FAILURE;
            ATBException atbException = new ATBException("DB INSERT FAILURE", errorCode, e.getCause());
            ...
            propMap.put(THeader.RECEIVER_ADAPTOR_NAME.key(), adaptorConfigBean.getAdaptorName());
            propMap.put(THeader.RECEIVER_RESULT_CD.key(), TResult.FAIL.value());
            propMap.put(THeader.RECEIVER_RESULT_MSG.key(), "DB INSERT FAILURE");
            retMap.put(THeader.IF_RESULT.key(), propMap);
            return retMap;
        }
