...
public abstract class OAuth2AccessTokenSupport {
	...
	public volatile RestTemplate restTemplate;
	...
	public RestTemplate getRestTemplate() {
		if (restTemplate == null) {
			synchronized (this) {
				if (restTemplate == null) {
					//log.info("interceptors : "+interceptors.toString());
	...
	protected OAuth2AccessToken retrieveToken(AccessTokenRequest request, OAuth2ProtectedResourceDetails resource,
		...
		try {
			...
			log.info( "retrieveToken Execute "); ;
	...
	protected String getAccessTokenUri(OAuth2ProtectedResourceDetails resource, MultiValueMap<String, String> form) {
		...
		if (log.isDebugEnabled()) {
			log.debug("Retrieving token from " + accessTokenUri);
		}
	...
	private class OAuth2AuthTokenCallback implements RequestCallback {
		...
		public void doWithRequest(ClientHttpRequest request) throws IOException {
			...
			if (log.isDebugEnabled()) {
				log.debug("Encoding and sending form: " + form);
			}
