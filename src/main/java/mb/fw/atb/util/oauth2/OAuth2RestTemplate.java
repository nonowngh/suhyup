package mb.fw.atb.util.oauth2;

import lombok.extern.slf4j.Slf4j;
import mb.fw.atb.util.oauth2.provider.AuthorizationCodeAccessTokenProvider;
import mb.fw.atb.util.oauth2.provider.ClientCredentialsAccessTokenProvider;
import mb.fw.atb.util.oauth2.provider.ImplicitAccessTokenProvider;
import mb.fw.atb.util.oauth2.provider.ResourceOwnerPasswordAccessTokenProvider;
...
public class OAuth2RestTemplate extends RestTemplate implements OAuth2RestOperations {
	...
	public AccessTokenProviderChain accessTokenProviderChain = new AccessTokenProviderChain(Arrays.<AccessTokenProvider> asList(
	...
	public AccessTokenProviderChain getAccessTokenProviderChain() {
			return accessTokenProviderChain;
	}
	...
	public OAuth2RestTemplate(OAuth2ProtectedResourceDetails resource) {
		...
		log.info("OAuth2RestTemplate init");
	}
	...
	/**
	 * API를 사용하다가 강제로 AccessToken을 발급받아야 할 경우 사용한다.
	 *
	 * @return
	 * @throws UserRedirectRequiredException
	 */
	public OAuth2AccessToken forcedAccessToken()
			throws UserRedirectRequiredException {
		return acquireAccessToken(context);
	}

	protected OAuth2AccessToken acquireAccessToken(OAuth2ClientContext oauth2Context)
			throws UserRedirectRequiredException {
		//log.info("acquireAccessToken");

		AccessTokenRequest accessTokenRequest = oauth2Context.getAccessTokenRequest();
		//log.info("accessTokenRequest");
		...
		accessToken = accessTokenProviderChain.obtainAccessToken(resource, accessTokenRequest);
		log.info("accessToken : " + accessToken.getClass().getCanonicalName());
	...
	public void setAccessTokenProvider(AccessTokenProviderChain accessTokenProvider) {
		this.accessTokenProviderChain = accessTokenProvider;
	...
	public void setClockSkew(int clockSkew) {
		...
		propagateClockSkewToAccessTokenProvider(clockSkew, this.accessTokenProviderChain);
	}
